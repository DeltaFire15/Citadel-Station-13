...
/mob/living/carbon/emp_act(severity)
	...
	if(HAS_TRAIT(src, TRAIT_ROBOTIC_ORGANISM))
		//EMPs fuck robots over. Up to ~11.5 corruption per EMP if hit by the full power. They also get up to 15 burn damage per EMP (up to 2.5 per limb)
		//Though, note that the burn damage is linear, while corruption is logarythmical, which means at lower severities you still get corruption, but far less burn
